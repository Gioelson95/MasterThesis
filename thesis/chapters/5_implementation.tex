...
As can be seen from the scheme, we did two different feature extraction processes, one for audio and the other for \gls{eda}, due to the evident differences in the data.
\\
For the next features explanation, will be used the song contained in the \gls{pmemo} dataset under the name \textit{4.mp3}, X Bitch from 21 Savage from the album "\textit{Savage mode}". Its waveform and audio specs can be seen in figure \ref{fig:audio_specs}.
\begin{figure}[h]
    \centering
